...
import type { InventoryItemV2 } from '../../../types/gameV2'
...
import type { WeaponTag, ArmorTag } from '../../../types/gameV2'
...
interface ModalProps {
  onClose: () => void
  existing?: InventoryItemV2
}

function AddItemModalV2({ onClose, existing }: ModalProps) {
  const addItem = useCharacterV2Store((s) => s.addItem)
  const updateItem = useCharacterV2Store((s) => s.updateItem)
  ...
  function handleSave() {
    ...
    const base: Omit<InventoryItemV2, 'id'> = {
      name: name.trim(),
...
function ItemCardV2({ item }: { item: InventoryItemV2 }) {
  const removeItem = useCharacterV2Store((s) => s.removeItem)
  const updateItemQuantity = useCharacterV2Store((s) => s.updateItemQuantity)
  const toggleEquipped = useCharacterV2Store((s) => s.toggleEquipped)
  const toggleBroken = useCharacterV2Store((s) => s.toggleBroken)
  const useItem = useCharacterV2Store((s) => s.useItem)
  ...
  return (
    <>
      {editing && <AddItemModalV2 existing={item} onClose={() => setEditing(false)} />}
...
export default function InventoryV2() {
  const inventory = useCharacterV2Store((s) => s.character.inventory ?? [])
  ...
  return (
    <div className="flex flex-col gap-3">
      {adding && <AddItemModalV2 onClose={() => setAdding(false)} />}
