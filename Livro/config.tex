...
% --- Section withoubreak --- %
\newcommand{\titlesection}[1]{{\Large\bfseries #1}\medbreak}
...
% --- Habilidade de Combate com função e requisitos explícitos ---
% #1 = Nome, #2 = Custo, #3 = Requisito, #4 = Ação, #5 = Propósito, #6 = Efeito
\newcommand{\combatSkillFull}[6]{%
    ...
    {\setlength{\fboxsep}{2pt}\small\scshape\fbox{#2\ PC}}\par%
    \vspace{2pt}%
    \noindent{\small\textbf{Requisito:} #3. \textbf{Ação:} #4.}\par%
    %  \noindent{\small\textbf{Propósito:} \textit{#5}}\par %
    \noindent{\small\textbf{Efeito:} #6}\par%
    \vspace{4pt}%
}
...
\newcommand{\spellCard}[8]{%
    ...
    \vspace{3pt}%
    % Cabeçalho: faixa preta com nome e tags
    \noindent
    \begin{tikzpicture}[baseline=(H.base), inner sep=0pt, outer sep=0pt]
        \fill[black!88] (0,-0.18em) rectangle (\linewidth,1.2em);
        \node[anchor=west, text=white, font=\bfseries\scshape\small, inner sep=0pt]
            (H) at (4pt,0.48em) {#1};
        \node[anchor=east, text=white, font=\fontsize{6.2pt}{6.2pt}\selectfont,
              inner sep=0pt]
            at (\linewidth-4pt,0.48em)
            {#2\enspace#8\enspace{\bfseries| DIV #3+}};
    \end{tikzpicture}%
    \par\vspace{1pt}%
    % Descrição
    \noindent{\fontsize{7pt}{8.5pt}\selectfont\itshape #4}\par
    \vspace{1pt}%
    % Tabela compacta sem bordas laterais
    {%
        ...
        \begin{tabular*}{\linewidth}{@{}
            ...
            >{\centering}p{0.9cm}
            p{3.0cm}
            @{\extracolsep{\fill}}p{4.8cm}
        @{}}%
        \noalign{\hrule height 0.7pt}%
        \rowcolor{black!10}%
        {\bfseries Nív} & {\bfseries Custo} & {\bfseries #7} & {\bfseries Efeito Adicional}\\
    ...
    \ifstrempty{#6}{}{%
        \vspace{1pt}%
        \noindent{\fontsize{6.2pt}{7.5pt}\selectfont\itshape #6}\par%
